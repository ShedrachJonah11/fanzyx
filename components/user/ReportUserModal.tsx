...
import { useReportUser } from "@/services/hooks/users";
...
export function ReportUserModal({
  ...
  const { run, loading } = useReportUser(username);

  const submit = async () => {
    if (!reason || loading) return;
    try {
      await run({ reason, detail: detail.trim() || undefined });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md surface-card p-6 flex flex-col gap-5 max-h-[90vh] overflow-y-auto">
        ...
        <fieldset className="flex flex-col gap-2">
          <legend className="text-xs font-medium text-white/70 mb-1">
            Reason
          </legend>
          {REASONS.map((r) => {
